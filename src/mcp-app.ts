...
function formatValue(value: number, metric: MetricName): string {
  switch (metric) {
    ...
    case "employeeCount":
      return `${value.toLocaleString("nl-NL")} gbr`;
    case "accountAge":
      return `${value} mnd`;
...
function updateDetailPanel(customer: Customer): void {
  const segmentClass = customer.segment.toLowerCase().replace(/[^a-z]/g, "");
  detailPanel.innerHTML = `
    <span class="detail-name">${customer.name}</span>
    <span class="detail-segment ${segmentClass}">${customer.segment}</span>
    <span class="detail-metric"><strong>${formatValue(customer.annualRevenue, "annualRevenue")}</strong> contract</span>
    <span class="detail-metric"><strong>${formatValue(customer.employeeCount, "employeeCount")}</strong></span>
    <span class="detail-metric"><strong>${customer.engagementScore}/100</strong> tevr.</span>
...
function resetDetailPanel(): void {
  detailPanel.innerHTML =
    '<span class="detail-placeholder">Beweeg over een punt voor details</span>';
}

// Create app instance
const app = new App({ name: "AFAS Klantoverzicht", version: "1.0.0" });
